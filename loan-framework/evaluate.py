...
for idx, row in test_df.iterrows():
    ...
    try:
        ...
        context_aware_results.append({
            ...
            'ca_matches_ground': (ca_result['stage_4_decision'] == 'APPROVED') == (applicant_data['loan_approved_actual'] == 1)
        })
    ...
    try:
        ...
        baseline_results.append({
            ...
            'bl_matches_ground': (bl_result['final_decision'] == 'APPROVED') == (applicant_data['loan_approved_actual'] == 1)
        })
    except Exception as e:
        baseline_results.append({
            'applicant_id': applicant_data['applicant_id'],
...
ca_consistency = sum(
    1 for _, row in ca_df.iterrows() 
    if ((row['ca_risk_score'] < 60 and row['ca_decision'] == 'APPROVED') or
        (row['ca_risk_score'] >= 60 and row['ca_decision'] == 'REJECTED'))
) / len(ca_df) if len(ca_df) > 0 else 0
