...
def stage_1_document_verification(state):
...
def stage_2_credit_scoring(state):
...
def stage_4_final_approval(state):
    ...
    criteria_met = (
        state['stage_1_verification_status'] == 'pass' and
        not state['bankruptcy_history'] and
        state['stage_3_risk_score'] < 60 and
        state['debt_to_income'] <= 0.55
    )
