...
from loan_framework.baseline.baseline_agents import run_baseline_pipeline_improved
from loan_framework.cso.agents import run_context_aware_pipeline

"""
PHASE 2: Evaluation on Test Data Using Simplified Data + Improved Agents

Uses discovered factors to evaluate test applicants with proper CSO flow.
Updated to work with simplified synthetic data and improved agent prompts.
"""


print("\n" + "=" * 80)
print("PHASE 2: TEST DATA EVALUATION WITH SIMPLIFIED DATA + IMPROVED AGENTS")
print("=" * 80)
...
# ============================================================================
# LOAD TEST DATA (Simplified Synthetic Data)
# ============================================================================
...
# ============================================================================
# HELPER FUNCTION: CREATE CSO FROM CSV ROW
# ============================================================================
...
def create_cso_from_csv_row(row, applicant_id):
    ...
    """
    cso = {
        # Layer 0: Applicant Information (37 fields from CSV)
...
print("Evaluating test applicants using discovered factors and improved agents...\n")
print("Progress:")
print("-" * 80)
...
for row_number, (_, row) in enumerate(test_df.iterrows()):
    ...
    if (row_number + 1) % max(1, len(test_df) // 10) == 0:
        ...
        progress_pct = (row_number + 1) / len(test_df) * 100
        print(
            f"  {progress_pct:6.1f}% | {row_number + 1:,}/{len(test_df):,} applicants | Elapsed: {elapsed:.1f}s"
        )
    ...
    try:
        stage_timer = time.time()
        bl_cso = run_baseline_pipeline_improved(cso.copy())
